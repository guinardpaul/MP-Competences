...
export class InitPasswordComponent implements OnInit {
    ...
    this._authService.initUserPassword(user)
      .subscribe(data => {
        if (data.success) {
          ...
          this._flashMsg.displayMsg('Erreur durant la mise à jour du password. Veuillez réessayer.',
            'alert-success', 3000);
        }
      }, err => console.log(err)
      );
  ...
  ngOnInit() {
    if (this._activatedRoute.snapshot.params[ '_id' ] !== undefined) {
      ...
      this._flashMsg.displayMsg('Ce lien a expiré. Entrez votre adresse E-mail pour recevoir un lien valide.',
        'alert-success', 3000);
